...
const addOpeningStock = (req, res) => {
    ...
    const { product_id, company_id, quantity, remarks } = req.body;

    if (!product_id || !company_id || quantity == null) {
        return res.status(400).json({
            success: false,
            message: "Product ID, Company ID and Quantity are required"
        });
    }

    getStockByProduct(product_id, req.user.id, (err, result) => {
...
const getAllStock = (req, res) => {
    ...
    getStock(req.user.id, (err, result) => {
...
const getProductStockHistory = (req, res) => {
    ...
    getStockHistory(req.params.productId, req.user.id, (err, result) => {
...
const stockIn = (req, res) => {
    ...
    increaseStock(product_id, req.user.id, quantity, (err) => {
...
const stockOut = (req, res) => {
    ...
    getStockByProduct(product_id, req.user.id, (err, result) => {
        ...
        decreaseStock(product_id, req.user.id, quantity, (err) => {
